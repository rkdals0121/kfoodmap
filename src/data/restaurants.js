// Curated places for K-Food Map — schema v2 (P0 data-integrity sprint).
//
// Generated by scripts/migrate-dietary-v2.mjs; safe to hand-edit from here on.
//
// Every factual field carries its provenance via fact(): a value, how far we
// trust it (STATUS) and where it came from (SOURCE). Nothing here has been
// confirmed against a primary source yet, so no field is VERIFIED — the app
// says so rather than implying certainty it does not have.
//
// Removed in v2 because they could not be justified (see the P0 audit):
//   • rating / reviews  — invented figures presented as real scores
//   • food_mile         — invented kilometre figures presented as ESG data
// They are not replaced with new guesses. Unknown is the honest answer.
//
// Dietary levels are never inferred: see the decision table (with evidence)
// in scripts/migrate-dietary-v2.mjs.

// Extension is explicit so data QA scripts can import this under plain Node.
import { fact, unknownFact, STATUS, SOURCE, VEGAN, HALAL } from './verification.js';

export const restaurants = [
  {
    id: "balwoo",
    name: "Balwoo Gongyang (발우공양)",
    zone: "Jongno, Seoul",
    category: "temple",

    coordinates: fact({ lat: 37.5737728, lng: 126.983174 }, { status: STATUS.ESTIMATED, source: SOURCE.GEOCODER, evidence: "Geocoder hit; not confirmed as the venue entrance" }),
    address: fact("56 Ujeongguk-ro, Jongno-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: fact("11:30 AM – 9:30 PM", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT }),
    menus: fact([
      { name: "Michelin Temple Course", price: "45,000 KRW" },
      { name: "Meditation Set Meal", price: "65,000 KRW" },
      { name: "Fermented Plum Tea", price: "~8,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "v1 tagged Vegan; Buddhist temple course menu (\"Michelin Temple Course\")" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Mild Taste","Fermented","Zero-waste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "Minimalist wood and hush — dining as meditation.",
    story: "Rooted in the ancient mindfulness of Korean Buddhist monks, Balwoo Gongyang treats every ingredient with profound reverence. Their artisanal fermentation techniques, honed over centuries, transform locally harvested mountain greens into deep, restorative flavors. The kitchen operates on a strict zero-waste philosophy, ensuring that nothing returned to the earth is without purpose.",
    esg_point: "Uses traditional fermentation without artificial additives",

    image: "/images/temple_food.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "sanchon",
    name: "Sanchon (산촌)",
    zone: "Insadong, Seoul",
    category: "temple",

    coordinates: fact({ lat: 37.5732644, lng: 126.9860943 }, { status: STATUS.ESTIMATED, source: SOURCE.GEOCODER, evidence: "Geocoder hit; not confirmed as the venue entrance" }),
    address: fact("30-13 Insadong-gil, Jongno-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: fact("12:00 PM – 9:00 PM", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT }),
    menus: fact([
      { name: "Traditional Temple Banchan Set", price: "33,000 KRW" },
      { name: "Wild Mountain Greens Rice", price: "22,000 KRW" },
      { name: "Lotus Root Pancakes", price: "~15,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "v1 tagged Vegan; temple banchan / mountain-greens menu" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Mild Taste","Fermented","Local Sourcing"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A lantern-lit Hanok hidden in Insadong's alleys.",
    story: "Sanchon embodies the purest form of agrarian Korean heritage. Founded by a former monk, the restaurant's daily offerings are dictated by what the earth yields. Ingredients are sustainably foraged from the pristine highlands of Gangwon-do. Every dish is a testament to artisanal patience, bringing the quiet, unadulterated essence of the forest straight to the table.",
    esg_point: "Locally sourced ingredients directly foraged from Gangwon-do mountains",

    image: "/images/temple_food.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "osegyehyang",
    name: "Osegyehyang (오세계향)",
    zone: "Insadong, Seoul",
    category: "korean-chinese",

    coordinates: fact({ lat: 37.5747, lng: 126.9853 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("14-5, Insadong 12-gil, Jongno-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: fact("11:30 AM – 9:00 PM", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT }),
    menus: fact([
      { name: "Vegan Jajangmyeon", price: "9,000 KRW" },
      { name: "Soy Meat Sweet and Sour", price: "18,000 KRW" },
      { name: "Spicy Vegan Jjamppong", price: "10,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"convert rich Korean-Chinese classics into 100% vegan meals\"" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A humble Korean-Chinese spot in an Insadong alley.",
    story: "Osegyehyang is a pioneer in bringing plant-based alternatives to beloved comfort foods. Famous for their brilliant soy meat dishes, they seamlessly convert rich Korean-Chinese classics into 100% vegan meals. This is the perfect spot to enjoy guilt-free, sustainable versions of everyday favorites.",
    esg_point: "100% plant-based takes on Korean-Chinese comfort classics",

    image: "/images/noodles.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "plant-cafe",
    name: "Plant Cafe & Kitchen",
    zone: "Itaewon, Seoul",
    category: "vegan-dining",

    coordinates: fact({ lat: 37.533, lng: 126.993 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("117, Bogwang-ro, Yongsan-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: fact("11:00 AM – 10:00 PM", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT }),
    menus: fact([
      { name: "Lentil Veggie Burrito Bowl", price: "14,500 KRW" },
      { name: "Avocado Burger", price: "15,500 KRW" },
      { name: "Vegan Chocolate Cake", price: "~8,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "v1 tagged Vegan; source describes \"hearty western-style vegan dining\" with an in-house vegan bakery" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A sunlit Itaewon hub with its own vegan bakery.",
    story: "A trailblazer for the plant-based community in Itaewon, Plant Cafe combines hearty western-style vegan dining with in-house bakery delights. Their lentil bowls and burgers prove that veganism can be incredibly indulgent. The vibrant space acts as a hub for eco-conscious expats and locals alike.",
    esg_point: "In-house vegan bakery cutting dairy and egg supply chains",

    image: "/images/vegan_cafe.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "monks-butcher",
    name: "Monk's Butcher",
    zone: "Itaewon, Seoul",
    category: "vegan-dining",

    coordinates: fact({ lat: 37.5355, lng: 126.9991 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Itaewon-ro, Yongsan-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Beyond Meat Burger", price: "~18,000 KRW" },
      { name: "Vegan Mushroom Risotto", price: "~20,000 KRW" },
      { name: "Plant-based Steak", price: "~28,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"modern, upscale vegan dining\"; menu is entirely plant-based" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "Plant-based fare, served like fine dining.",
    story: "Elevating the concept of a 'fake meat' eatery, Monk's Butcher brings modern, upscale vegan dining to the heart of Itaewon. Their sophisticated plant-based twists on classic butcher shop fare challenge culinary boundaries. Enjoy an elegant evening where sustainability meets high-end gastronomy.",
    esg_point: "High-end plant proteins replacing butcher-shop staples",

    image: "/images/temple_food.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "camouflage",
    name: "Camouflage",
    zone: "Itaewon, Seoul",
    category: "vegan-dining",

    coordinates: fact({ lat: 37.533, lng: 126.996 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Itaewon-dong, Yongsan-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Vegan Kung Pao Chick'n", price: "~19,000 KRW" },
      { name: "Plant-based Chow Mein", price: "~16,000 KRW" },
      { name: "Crispy Mushroom Bites", price: "~12,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: menu is \"completely free of animal products\"" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "Bold Chinese-American flavors that happen to be vegan.",
    story: "Camouflage creatively 'hides' the fact that its bold Chinese-American menu is completely free of animal products. By mastering textures and savory sauces, dishes like their vegan Kung Pao Chick'n fool the senses. It is a brilliant example of how playful and satisfying sustainable cuisine can be.",
    esg_point: "Bold sauces and textures that make plant-based eating effortless",

    image: "/images/vegan_cafe.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "eid",
    name: "EID Halal Korean Food",
    zone: "Itaewon, Seoul",
    category: "halal-korean",

    coordinates: fact({ lat: 37.5332, lng: 126.9967 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("67, Usadan-ro 10-gil, Yongsan-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: fact("11:30 AM – 9:00 PM", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT }),
    menus: fact([
      { name: "Halal Bulgogi", price: "~15,000 KRW" },
      { name: "Halal Samgyetang", price: "~18,000 KRW" },
      { name: "Halal Bibimbap", price: "~10,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: unknownFact("No vegan information in the source"),
    halal: fact(HALAL.FRIENDLY, { status: STATUS.ESTIMATED, source: SOURCE.SELF_DECLARED, evidence: "Venue name declares halal (\"EID Halal Korean Food\")" }),
    // Unverified certification claim — never treated as HALAL.CERTIFIED.
    halalCertClaim: {"body":"KMF (Korea Muslim Federation)","status":"claimed in project draft; certificate not sighted"},
  },
    traits: ["Mild Taste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A warm, family-run dining room near the Seoul Central Mosque.",
    story: "Run by a dedicated Korean Muslim family, EID was one of the first KMF-certified restaurants in Seoul. They offer the comfort of home-cooked Korean meals, like mild Bulgogi and Samgyetang, with absolute Halal integrity. This warm, welcoming space truly bridges Korean heritage and Islamic dietary laws.",
    esg_point: "KMF-certified family kitchen serving inclusive Korean home cooking",

    image: "/images/halal_meat.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "makan",
    name: "Makan Halal Korean Restaurant",
    zone: "Itaewon, Seoul",
    category: "halal-korean",

    coordinates: fact({ lat: 37.536, lng: 126.993 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Near Seoul Central Mosque, Itaewon, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Halal Braised Chicken (Jjimdak)", price: "~10,000 KRW" },
      { name: "Halal Grilled Mackerel", price: "~10,000 KRW" },
      { name: "Halal Tteokbokki", price: "~8,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: unknownFact("No vegan information in the source"),
    halal: fact(HALAL.FRIENDLY, { status: STATUS.ESTIMATED, source: SOURCE.SELF_DECLARED, evidence: "Venue name declares halal (\"Makan Halal Korean Restaurant\")" }),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A casual canteen steps from the mosque, loved by Muslim travelers.",
    story: "Located just steps from the Seoul Central Mosque, Makan provides an accessible gateway into local cuisine. Their highly affordable menu spans from spicy stews to grilled fish, all meticulously prepared. Muslim travelers can explore authentic, fiery Korean flavors with complete peace of mind here.",
    esg_point: "Affordable Halal access to everyday Korean dishes",

    image: "/images/halal_meat.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "kampungku",
    name: "Kampungku",
    zone: "Itaewon, Seoul",
    category: "halal-korean",

    coordinates: fact({ lat: 37.5332, lng: 126.9976 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("39, Usadan-ro 10-gil, Yongsan-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Nasi Lemak", price: "~12,000 KRW" },
      { name: "Halal Bibimbap", price: "~9,000 KRW" },
      { name: "Korean Fried Chicken", price: "~16,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: unknownFact("No vegan information in the source"),
    halal: fact(HALAL.FRIENDLY, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states menus are prepared \"under a strict Halal standard\"" }),
  },
    traits: ["Mild Taste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A cozy Malaysian-Korean fusion spot on Usadan-ro's halal street.",
    story: "Kampungku represents a beautiful culinary fusion of Korean and Malaysian traditions under a strict Halal standard. Patrons can effortlessly switch between a comforting bowl of Nasi Lemak and a sizzling Korean Bibimbap. This harmonious cross-cultural menu highlights the global adaptability of K-Food.",
    esg_point: "Cross-cultural Halal menu bridging Korean and Malaysian tables",

    image: "/images/halal_meat.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "nono-shop",
    name: "Nono Shop & Cafe",
    zone: "Yongsan, Seoul",
    category: "zero-waste",

    coordinates: fact({ lat: 37.533, lng: 126.991 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Yongsan-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Vegan Croissant", price: "~5,500 KRW" },
      { name: "Oat Milk Latte", price: "~6,000 KRW" },
      { name: "Zero Waste Nut Mix", price: "~7,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"Every vegan product and beverage sold here\"" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Zero-waste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "Part cafe, part refill shop — a tiny zero-waste lifestyle hub.",
    story: "More than just a cafe, Nono Shop is a lifestyle space fiercely dedicated to environmental preservation. Every vegan product and beverage sold here adheres to a strict zero-waste philosophy, refusing single-use plastics. Visitors can nourish their bodies while actively reducing their ecological footprint.",
    esg_point: "Single-use plastic free; every item vegan and package-free",

    image: "/images/vegan_cafe.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "ggot-epida",
    name: "Ggot, E-Pi Da (꽃, 에피다)",
    zone: "Bukchon, Seoul",
    category: "zero-waste",

    coordinates: fact({ lat: 37.5802, lng: 126.9775 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("15-2 Bukchon-ro 5-gil, Jongno-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Seasonal Whole-Vegetable Course", price: "~38,000 KRW" },
      { name: "Root-to-Leaf Salad", price: "~16,000 KRW" },
      { name: "Omija Tea", price: "~7,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: unknownFact("Source describes whole-plant cooking but never states the kitchen is vegan — not inferred"),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Mild Taste","Zero-waste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A sunlit courtyard of old roof tiles and artisan ceramics.",
    story: "This artisanal haven breathes new life into the concept of whole-plant culinary art. Rejecting the modern throwaway culture, the chefs meticulously utilize every stem, root, and leaf to craft delicate, mild-tasting masterpieces. Their dedication to zero-waste cooking elevates humble, organic produce into an elegant celebration of sustainable Korean gastronomy.",
    esg_point: "Zero-waste initiative focusing on utilizing every part of the organic vegetable",

    image: "/images/mild_soup.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "maji",
    name: "Maji (마지)",
    zone: "Seochon, Seoul",
    category: "temple",

    coordinates: fact({ lat: 37.5786, lng: 126.9691 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("19 Jahamun-ro 10-gil, Jongno-gu, Seoul", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Temple-style Jang Course", price: "~35,000 KRW" },
      { name: "Fermented Soybean Stew", price: "~14,000 KRW" },
      { name: "Seasonal Namul Plate", price: "~12,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"a fully plant-based menu\"" }),
    halal: unknownFact("v1 Halal tag was derived from the plant-based menu; certification never evidenced — inference removed"),
  },
    traits: ["Fermented"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A tranquil sanctuary of paper doors and pine wood.",
    story: "Maji stands as a pinnacle of inclusive, sustainable dining, offering a fully plant-based menu that naturally adheres to Halal standards. The soul of their cuisine lies in traditional 'Jang' (soy paste and sauce), fermented naturally in breathing clay pots for over three years. This uncompromising artisanal dedication guarantees a profoundly rich, earth-conscious culinary journey.",
    esg_point: "Plant-based inclusivity with naturally fermented soy sauces aged over 3 years",

    image: "/images/fermented_dish.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "chaeyuk-songdo",
    name: "Chaeyuk Sikdang Songdo (채육식당 송도점)",
    zone: "Songdo, Incheon",
    category: "vegan-dining",

    coordinates: fact({ lat: 37.383, lng: 126.637 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("5, Technopark-ro 111beon-gil, Yeonsu-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: fact("10:00 AM – 10:00 PM", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT }),
    menus: fact([
      { name: "Soy-Meat Jeyuk Set", price: "13,500 KRW" },
      { name: "Mushroom Bulgogi Set", price: "~14,000 KRW" },
      { name: "Unlimited Namul Bar", price: "Included" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states pork is replaced with soy meat as a \"vegan alternative\"" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Mild Taste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A bright, cafeteria-style spot with an unlimited fresh namul bar.",
    story: "Chaeyuk Sikdang cleverly replaces pork in the classic Korean Jeyuk Bokkeum with high-quality soy meat, creating a surprisingly realistic vegan alternative. Their mild, savory marinades are accompanied by a generous self-serve bar of fresh salads and namul. It's a bountiful, healthy take on a ubiquitous worker's lunch.",
    esg_point: "Soy-meat jeyuk and a self-serve namul bar cutting food waste",

    image: "/images/vegan_cafe.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "iryonghal",
    name: "Iryonghal Yangsik (일용할양식)",
    zone: "Michuhol, Incheon",
    category: "brunch-bakery",

    coordinates: fact({ lat: 37.451, lng: 126.701 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Michuhol-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Signature Ratatouille", price: "~18,000 KRW" },
      { name: "Vegetable-forward Brunch Plate", price: "~16,000 KRW" },
      { name: "Fresh Tomato Bruschetta", price: "~10,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.OPTIONS, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source says \"vegetable-forward\" and \"meatless\", not vegan — dairy/egg unclear, so the stronger claim is not made" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Mild Taste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A Blue Ribbon brunch cafe where vegetables star.",
    story: "Awarded the prestigious Blue Ribbon, this vegetable-forward brunch cafe elevates simple ingredients to art. Their signature ratatouille emphasizes the mild, intrinsic sweetness of slow-cooked local produce. It stands as a testament to the fact that healthy, meatless meals can achieve culinary excellence in Incheon.",
    esg_point: "Vegetable-forward menu built on slow-cooked local produce",

    image: "/images/mild_soup.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "rim",
    name: "Rim (림)",
    zone: "Cheongna, Incheon",
    category: "vegan-dining",

    coordinates: fact({ lat: 37.534, lng: 126.635 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Cheongna, Seo-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Vegan Lasagna", price: "~22,000 KRW" },
      { name: "Vegan Gnocchi", price: "~19,000 KRW" },
      { name: "Basil Pesto Pasta", price: "~18,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"100% plant-based Italian cuisine\"" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "An intimate Italian dining room, plant-based from sauce to dessert.",
    story: "Rim proves that 100% plant-based Italian cuisine can rival its traditional counterparts. By crafting approximately 98% of their rich sauces entirely in-house, they ensure ultimate freshness and vegan integrity. Their lasagna and gnocchi provide deep, comforting layers of flavor without relying on dairy.",
    esg_point: "98% of sauces made in-house, fully dairy-free",

    image: "/images/pasta.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "meat-morning",
    name: "Meat Morning (밋모닝)",
    zone: "Michuhol, Incheon",
    category: "brunch-bakery",

    coordinates: fact({ lat: 37.448, lng: 126.702 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Michuhol-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Sugar/Butter-free Cookies", price: "~4,000 KRW" },
      { name: "Gluten-free Bread", price: "~6,000 KRW" },
      { name: "Homemade Vegan Greek Yogurt", price: "~7,500 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.FULL, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"vegan desserts\" and \"homemade plant-based Greek yogurt\"" }),
    halal: unknownFact("No halal information in the source"),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A quiet bakery counter of sugar-free, butter-free small-batch treats.",
    story: "Despite its somewhat ironic name, Meat Morning is actually a haven for health-conscious vegan desserts. They meticulously bake sugar-free and butter-free cookies alongside artisan gluten-free breads. Their homemade plant-based Greek yogurt offers a creamy, probiotic-rich treat for mindful eaters.",
    esg_point: "Sugar-free, butter-free baking in small daily batches",

    image: "/images/vegan_cafe.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "arabesque",
    name: "Arabesque",
    zone: "Yeonsu, Incheon",
    category: "world-halal",

    coordinates: fact({ lat: 37.409, lng: 126.667 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Yeonsu-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Halal Lamb Kebab", price: "~18,000 KRW" },
      { name: "Chicken Tikka Masala", price: "~15,000 KRW" },
      { name: "Fresh Hummus & Pita", price: "~9,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: unknownFact("No vegan information in the source"),
    halal: fact(HALAL.FRIENDLY, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Source states: \"strictly Halal, alcohol-free Indian and Turkish delicacies\"" }),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "Tandoor-warmed, serving Incheon's Muslims since 2003.",
    story: "A historic pillar in Incheon since 2003, Arabesque serves strictly Halal, alcohol-free Indian and Turkish delicacies. Their enduring presence highlights the city's long-standing multiculturalism. Diners are treated to authentic, spice-rich meats cooked in traditional tandoors.",
    esg_point: "Two decades of strictly Halal, alcohol-free kitchens",

    image: "/images/halal_meat.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "bombay-brau",
    name: "Bombay Brau",
    zone: "Songdo, Incheon",
    category: "world-halal",

    coordinates: fact({ lat: 37.3918, lng: 126.6448 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Songdo, Yeonsu-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Vegan Dal Makhani", price: "~14,000 KRW" },
      { name: "Halal Tandoori Chicken", price: "~20,000 KRW" },
      { name: "Vegetable Samosas", price: "~8,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.OPTIONS, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu lists \"Vegan Dal Makhani\" alongside \"Halal Tandoori Chicken\" — vegan dishes exist, the kitchen is not vegan" }),
    halal: fact(HALAL.FRIENDLY, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu lists \"Halal Tandoori Chicken\"; source states it caters to halal diets" }),
  },
    traits: [],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "Rich curries shared with no compromise, halal or vegan.",
    story: "Nestled in the international district of Songdo, Bombay Brau caters brilliantly to both Halal and Vegan diets. Their expansive menu guarantees that diverse groups can dine together without compromising their beliefs. The rich curries here showcase the intense, sustainable power of plant and certified-meat cooking.",
    esg_point: "One table welcoming Halal and Vegan diners alike",

    image: "/images/halal_meat.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "gonghwachun",
    name: "Gonghwachun (공화춘)",
    zone: "Chinatown, Incheon",
    category: "korean-chinese",

    coordinates: fact({ lat: 37.4754, lng: 126.6196 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("Chinatown, Jung-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Area-level only — no street address on file", precision: "area" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Original Jajangmyeon", price: "~10,000 KRW" },
      { name: "Mild White Jjamppong", price: "~12,000 KRW" },
      { name: "Sweet and Sour Pork", price: "~25,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: fact(VEGAN.NONE, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu lists \"Sweet and Sour Pork\"" }),
    halal: fact(HALAL.NONE, { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu lists \"Sweet and Sour Pork\"" }),
  },
    traits: ["Mild Taste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "The century-old birthplace of jajangmyeon in Incheon's Chinatown.",
    story: "Steeped in history, Gonghwachun is widely celebrated as the birthplace of Jajangmyeon in Incheon's Chinatown. Their legendary black bean noodles boast a mild, savory depth that has defined Korean-Chinese cuisine for a century. Eating here is a direct taste of a vital piece of Korea's modern culinary heritage.",
    esg_point: "A century of culinary heritage preserved in one bowl",

    image: "/images/noodles.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },

  {
    id: "akiya",
    name: "Akiya (아키야)",
    zone: "Gaehang-ro, Incheon",
    category: "local-seasonal",

    coordinates: fact({ lat: 37.4764, lng: 126.6225 }, { status: STATUS.ESTIMATED, source: SOURCE.AREA_FALLBACK, evidence: "Neighbourhood centre used because geocoding did not resolve — may be off by ~100 m" }),
    address: fact("12 Gaehang-ro, Jung-gu, Incheon", { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, precision: "street" }),
    hours: unknownFact("Opening hours never confirmed"),
    menus: fact([
      { name: "Seasonal Donburi", price: "~15,000 KRW" },
      { name: "Local Greens Set", price: "~13,000 KRW" },
      { name: "House Tea", price: "~5,000 KRW" },
    ], { status: STATUS.ESTIMATED, source: SOURCE.DRAFT, evidence: "Menu names and prices from the draft; most prices are approximate" }),

    dietary: {
    vegan: unknownFact("No vegan information in the source"),
    halal: unknownFact("No halal information in the source"),
  },
    traits: ["Local Sourcing","Mild Taste"],

    // Editorial copy from the project draft; claims inside are not verified.
    vibe: "A restored 1920s wooden house, warm and quietly Zen.",
    story: "Nestled in the historic Gaehang-ro district, Akiya acts as a culinary bridge between Incheon's rich past and a sustainable future. The kitchen's philosophy revolves around ultra-local sourcing, reducing food mileage to a mere 5 kilometers. Their artisanal approach highlights the natural, unmasked flavors of the season, honoring the tireless work of local farmers.",
    esg_point: "Empowers local Incheon farmers by strictly purchasing seasonal yields",

    image: "/images/mild_soup.svg",
    photo: null,
    coverImage: null,
    gallery: [],
  },
];
