...
- ✅ **나머지 화면 문자열 추출 완료 (2026-09-18)** — FilterBar·
  ...
  (HANDOFF §7 #27)
- ✅ **i18n 2차 패스 (2026-09-18)** — provenance 문단의 6개 문장-조각 키를
  `<Trans>` 3문장 키로 교체(문장 순서를 바꿀 수 없던 문제 해결, 영어 출력은
  그대로), 날짜 포맷이 `src/utils.js`의 `DATE_LOCALES`를 통해 활성 언어를
  따라가도록(`'en' → 'en-GB'`, 오늘 출력 불변), 전수 키 검증 스캔이
  `t()`뿐 아니라 `i18nKey`까지, 단수(`count:1`)뿐 아니라 복수
  `_other`까지 커버하도록 강화. **문구 추출은 완료, 번역 콘텐츠는 여전히
  0**(검증 가능 인력 대기, HANDOFF §7 #27). 게이트 실측: `npm test` 48개
  통과, `npm run lint` 기존 경고 1개 유지, `npm run check-data` 0건
